...
Chrome extension for [CurrentRMS](https://www.current-rms.com/) — multi-store stock checker, quote & crew dashboards, and quote mute system.
...
### Quote Mute System *(new in v1.4.0)*
...
10. Click **Open Quote Dashboard** or use the "Quote Dashboard" tab in the CurrentRMS navigation bar
...
Update the version number in three places:
1. `version.json` — update `version` and `changelog`
2. `popup.js` — update `CURRENT_VERSION` constant
3. `manifest.json` — update `version`

Push to the `main` branch. All installations will see the update notification next time the popup is opened.
