...
(function init() {
  ...
  var licenseResolved = false;
  function onLicenseResult(s) {
    if (licenseResolved) return;
  ...
  chrome.runtime.sendMessage({ action: 'getLicenseStatus' }, function (status) {
    ...
    onLicenseResult(status || { allowed: false });
  });
  setTimeout(function () { onLicenseResult({ allowed: true }); }, 2000);
  function run() {
  var loadingOverlayEl = document.getElementById('loadingOverlay');
  var hideLoading = function () { if (loadingOverlayEl) loadingOverlayEl.classList.add('hidden'); };
  ...
  loadCredentials().then(async () => {
    try {
      ...
      startClock();
      if (subdomain && apiKey) {
        if (loadingText) loadingText.textContent = 'Loading service types...';
      ...
      console.error('[Crew Dashboard] Init error:', e);
      if (pollStatus) pollStatus.textContent = 'Error: ' + (e.message || e);
    } finally {
      if (loadingOverlay) loadingOverlay.classList.add('hidden');
    }
  }).catch(function (e) {
    console.error('[Crew Dashboard] Load error:', e);
    if (loadingOverlay) loadingOverlay.classList.add('hidden');
    if (pollStatus) pollStatus.textContent = 'Failed to load. Check connection.';
  });
  ...
  function loadLogo() {
    ...
    if (!logoMark) return;
    const img = new Image();
    img.onload = () => { try { logoMark.innerHTML = ''; logoMark.appendChild(img); } catch (e) { console.warn('[Crew] Logo load:', e); } };
  ...
  // ── Credentials (with timeout so overlay never sticks) ─────
  function loadCredentials() {
    ...
    if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.sync) {
      chrome.storage.sync.get(['subdomain', 'apiKey'], function (r) {
        clearTimeout(t);
        subdomain = (r && r.subdomain) || '';
        apiKey = (r && r.apiKey) || '';
        finish();
      });
    } else {
  ...
  function startClock() {
    function tick() {
      ...
      if (clockDateEl) clockDateEl.textContent = now.toLocaleDateString([], { weekday: 'long', day: 'numeric', month: 'short', year: 'numeric' });
    }
    tick();
    if (clockEl || clockDateEl) setInterval(tick, 10000);
  }
...
<span style="color:#4a4a5a;">To adjust alert settings, open the dashboard and click Settings.</span>
  ...
  async function pollOnce() {
    if (!subdomain || !apiKey) {
      if (pollStatus) pollStatus.textContent = 'No API credentials';
      if (pollDot) { pollDot.style.background = 'var(--error)'; pollDot.style.boxShadow = '0 0 6px var(--error)'; }
    ...
    if (config.departments.length === 0) {
      ...
      if (pollStatus) pollStatus.textContent = 'No rules set';
      return;
    }
    if (isPolling) return;
    isPolling = true;
    if (pollStatus) pollStatus.textContent = 'Scanning...';
    ...
    if (pollDot) { pollDot.style.animation = 'none'; pollDot.style.background = 'var(--warning)'; pollDot.style.boxShadow = '0 0 6px var(--warning)'; }

    try {
      const opportunities = await fetchUpcomingOpportunities();
      if (statScanned) statScanned.textContent = opportunities.length;
      ...
      renderBoard();
      if (pollStatus) pollStatus.textContent = `Loading 0/${opportunities.length}...`;
      ...
      for (let i = 0; i < opportunities.length; i += batchSize) {
        ...
        if (pollStatus) pollStatus.textContent = `Loading ${done}/${opportunities.length}...`;
        if (statTotal) statTotal.textContent = deptResults.reduce((s, dr) => s + dr.quotes.length, 0);
      }
      ...
      const now = new Date();
      if (pollStatus) pollStatus.textContent = `Updated ${now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
      if (pollDot) { pollDot.style.background = 'var(--accent)'; pollDot.style.boxShadow = '0 0 6px var(--accent)'; pollDot.style.animation = 'pulse 2s infinite'; }
    } catch (err) {
      console.error('Poll error:', err);
      if (pollStatus) pollStatus.textContent = 'Error — retrying...';
      if (pollDot) { pollDot.style.background = 'var(--error)'; pollDot.style.boxShadow = '0 0 6px var(--error)'; }
  ...
  function getTimeInputSec(valueEl, unitEl) {
    if (!valueEl || !unitEl) return 0;
    return Math.round((parseFloat(valueEl.value) || 0) * (parseInt(unitEl.value) || 3600));
  }
  ...
  if (addDeptBtn) addDeptBtn.addEventListener('click', () => addDeptBlock());
  if (stageFilter) stageFilter.addEventListener('click', e => { const c = e.target.closest('.stage-chip'); if (c) c.classList.toggle('active'); });
  ...
  function openSettings() {
    if (!pollIntervalSelect || !stageFilter || !deptRulesContainer) return;
    ...
    if (emailServiceId) emailServiceId.value = config.email.serviceId || '';
    if (emailTemplateId) emailTemplateId.value = config.email.templateId || '';
    if (emailPublicKey) emailPublicKey.value = config.email.publicKey || '';
    if (emailTo) emailTo.value = config.email.to || '';
    ...
    if (emailTestStatus) emailTestStatus.textContent = '';
  ...
  function applySettings() {
    ...
    config.email = {
      serviceId: (emailServiceId && emailServiceId.value) ? emailServiceId.value.trim() : '',
      templateId: (emailTemplateId && emailTemplateId.value) ? emailTemplateId.value.trim() : '',
      publicKey: (emailPublicKey && emailPublicKey.value) ? emailPublicKey.value.trim() : '',
      to: (emailTo && emailTo.value) ? emailTo.value.trim() : '',
  ...
  if (calToday) calToday.addEventListener('click', () => { const now = new Date(); calMonth = now.getMonth(); calYear = now.getFullYear(); renderCalendar(); });
  ...
  function renderCalendar() {
    ...
    if (!lastResults.length) {
      ...
      if (calMonthLabel) calMonthLabel.textContent = '';
      return;
    }

    const months = ['January','February','March','April','May','June','July','August','September','October','November','December'];
    if (calMonthLabel) calMonthLabel.textContent = `${months[calMonth]} ${calYear}`;

    // Build dept filter chips
    if (calDeptFilters) calDeptFilters.innerHTML = '';
    lastResults.forEach(dr => {
      const chip = document.createElement('div');
      chip.className = 'cal-dept-chip' + (calActiveDepts.size === 0 || calActiveDepts.has(dr.dept.name) ? ' active' : '');
      chip.style.background = calActiveDepts.size === 0 || calActiveDepts.has(dr.dept.name) ? hexToRgba(dr.dept.color || '#6b6b80', 0.15) : 'transparent';
      chip.style.color = dr.dept.color || '#6b6b80';
      chip.style.borderColor = dr.dept.color || '#6b6b80';
      ...
      if (calDeptFilters) calDeptFilters.appendChild(chip);
    });
  ...
  function buildCalCell(dayNum, dateKey, events, isOtherMonth, isToday) {
    ...
    let eventsHtml = '';
    const fallbackColor = '#6b6b80';
    unique.forEach(ev => {
      const items = ev.quote.serviceItems && Array.isArray(ev.quote.serviceItems) ? ev.quote.serviceItems : [];
      const totalNeeded = items.reduce((s, i) => s + (i.remaining != null ? i.remaining : 0), 0);
      ...
      const color = (ev.dept && ev.dept.color) ? ev.dept.color : fallbackColor;
      eventsHtml += `<div class="cal-event" style="background:${hexToRgba(color, 0.12)};color:${esc(color)};border-left-color:${esc(color)};" data-tooltip="${tooltipData}" data-opp-id="${ev.quote.id}"><span class="cal-ev-count">${Math.round(totalNeeded)}</span> ${esc(truncate(ev.quote.name, 22))}</div>`;
    });
  ...
  function hexToRgba(hex, alpha) {
    if (hex == null || typeof hex !== 'string') return 'rgba(0,0,0,' + (alpha != null ? alpha : 0.15) + ')';
    const h = hex.replace('#', '');
    if (h.length < 6) return 'rgba(0,0,0,' + (alpha != null ? alpha : 0.15) + ')';
    const r = parseInt(h.substring(0,2), 16) || 0;
    const g = parseInt(h.substring(2,4), 16) || 0;
    const b = parseInt(h.substring(4,6), 16) || 0;
    return 'rgba(' + r + ',' + g + ',' + b + ',' + (alpha != null ? alpha : 0.15) + ')';
  }

  // Calendar tooltip element
  const calTip = document.createElement('div');
  calTip.className = 'cal-tooltip';
  if (document.body) document.body.appendChild(calTip);
