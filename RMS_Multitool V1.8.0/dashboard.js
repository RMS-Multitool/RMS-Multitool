...
(function init() {
  ...
  loadCredentials().then(async () => {
    try {
      ...
      startClock();
      if (subdomain && apiKey) {
        if (loadingText) loadingText.textContent = 'Loading product groups from CurrentRMS...';
      ...
      console.error('[Quote Dashboard] Init error:', e);
      if (pollStatus) pollStatus.textContent = 'Error: ' + (e.message || e);
    } finally {
      if (loadingOverlay) loadingOverlay.classList.add('hidden');
    }
  }).catch(function (e) {
    console.error('[Quote Dashboard] Load error:', e);
    if (loadingOverlay) loadingOverlay.classList.add('hidden');
...
<span style="color:#4a4a5a;">To adjust alert settings, open the dashboard and click Settings.</span>
    ...
    try {
      ...
      for (let i = 0; i < opportunities.length; i += batchSize) {
        ...
        await Promise.all(batch.map(async opp => {
          try {
            console.log('[Dashboard] Opp date fields:', { id: opp.id, document_date: opp.document_date, created_at: opp.created_at, number: opp.number });
            ...
            deptResults.forEach(dr => {
              ...
              if (matches.length > 0) {
                dr.quotes.push({
                  ...
                  documentDate: opp.document_date || opp.created_at || '',
      ...
      if (statTotal) statTotal.textContent = totalQuotes;
      const now = new Date();
      ...
      if (pollStatus) pollStatus.textContent = `Error: ${err.message || err}`;
      if (pollDot) { pollDot.style.background = 'var(--error)'; pollDot.style.boxShadow = '0 0 6px var(--error)'; }
  ...
  function renderCard(q, dept, idx) {
    ...
    const isExpired = q.documentDate && (Date.now() - new Date(q.documentDate).getTime()) > 15 * 24 * 60 * 60 * 1000;
    const expiredDays = isExpired ? Math.floor((Date.now() - new Date(q.documentDate).getTime()) / (24 * 60 * 60 * 1000)) : 0;
    ...
    return `<div class="quote-card" style="animation-delay:${idx * 40}ms;" data-tooltip-items='${tooltipData.replace(/'/g, "&#39;")}'>
      ...
      ${isExpired ? `<div class="card-expired-banner"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>Quote expired — ${expiredDays} days old</div>` : ''}
      <div class="card-top">
        <div>
