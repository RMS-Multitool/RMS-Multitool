...
(() => {
  ...
  // ── Init — Mute/Hide only run when user has entered a trial or unlock code ─
  function init() {
  ...
  function updatePageTotal() {
    ...
    if (totalCell) {
      ...
      if (totalSpan) {
        ...
        if (mutedChargeSum > 0) {
          // Capture the real total once when we first need to override it
          if (originalTotal === null) {
            const raw = totalSpan.textContent.replace(/[^0-9.\-]/g, '');
            originalTotal = parseFloat(raw) || 0;
          }
          ...
          // Nothing muted — leave the real DOM value alone, just clean up indicators
          originalTotal = null;
  ...
  function updateRevenuePanel(mutedCharge, mutedTax) {
    ...
    for (const [id, subtract] of Object.entries(overrides)) {
      ...
      if (subtract > 0) {
        // Capture the real value once when we first need to override it
        if (originalRevenue[id] === undefined) {
          const raw = el.textContent.replace(/[^0-9.\-]/g, '');
          originalRevenue[id] = parseFloat(raw) || 0;
        }
        ...
        // Nothing muted — leave the real DOM value alone, reset cache for next time
        delete originalRevenue[id];
        el.style.color = '';
