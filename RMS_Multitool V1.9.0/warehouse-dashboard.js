...
(() => {
  ...
  function run() {
    ...
    async function loadData() {
      ...
      const now = new Date();
      // Show upcoming work: anything starting from ~yesterday onwards
      const futureStartIso = new Date(now.getTime() - 1 * 24 * 60 * 60 * 1000).toISOString();
      // Load more jobs while still staying under API limits (multiple list pages, but cap detailed loads).
      const LIST_PAGE_SIZE = 40;
      const MAX_JOBS_TO_LOAD = 50;
      const MAX_LIST_PAGES = 3;
      let allOpps = [];
      try {
        let page = 1;
        while (page <= MAX_LIST_PAGES && allOpps.length < MAX_JOBS_TO_LOAD) {
          const params = [
            ['per_page', String(LIST_PAGE_SIZE)],
            ['page', String(page)],
            ['q[starts_at_gteq]', futureStartIso],
            ['q[s][]', 'starts_at asc'],
            ['include[]', 'custom_fields']
          ];
          // Only include active jobs: 3=reserved, 4=order
          [3, 4].forEach(function (stateCode) { params.push(['q[state_in][]', String(stateCode)]); });
          const listUrl = buildApiUrl('opportunities', params);
          const data = await apiFetch(listUrl);
          const opps = data.opportunities || [];
          allOpps = allOpps.concat(opps);
          if (opps.length < LIST_PAGE_SIZE) break; // no more pages
          page += 1;
        }
        allOpps = allOpps.slice(0, MAX_JOBS_TO_LOAD);
      } catch (e) {
        setLoaded();
      ...
      allOpps.forEach(opp => {
        ...
        if ((allComplete || allBooked) && hasReadyDate) {
          ...
          item = { opp, progress, html, readyDate: readyVal };
      ...
      // Sort by ready-for-prep date (earliest first) where available
      function sortByReadyDate(arr) {
        arr.sort(function (a, b) {
          const ad = a && a.readyDate ? new Date(a.readyDate) : null;
          const bd = b && b.readyDate ? new Date(b.readyDate) : null;
      ...
      sortByReadyDate(readyToPrep);
      sortByReadyDate(inPrep);
      sortByReadyDate(readyToLoad);
