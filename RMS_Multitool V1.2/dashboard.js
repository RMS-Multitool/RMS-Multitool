...
<span style="color:#4a4a5a;">To adjust alert settings, open the dashboard and click Settings.</span>
  ...
  async function pollOnce() {
    ...
    try {
      ...
      for (let i = 0; i < opportunities.length; i += batchSize) {
        ...
        if (i + batchSize < opportunities.length) await sleep(300);
      }
      ...
      // Track first-seen and prune stale entries
      const activeKeys = [];
      deptResults.forEach(dr => {
        dr.quotes.forEach(q => {
  ...
  function renderBoard() {
    ...
    const activeDepts = lastResults.filter(dr => dr.quotes.length > 0);
    const totalQuotes = activeDepts.reduce((s, dr) => s + dr.quotes.length, 0);
    statTotal.textContent = totalQuotes;
    if (totalQuotes === 0) { showAllClearBoard(); return; }
